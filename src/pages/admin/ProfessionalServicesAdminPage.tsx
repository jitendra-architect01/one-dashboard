...
export default function ProfessionalServicesAdminPage() {
  ...
  const kpis = businessUnits.professional_services?.kpis || [];
  const initiatives = businessUnits.professional_services?.initiatives || [];

  const handleSaveKPI = (kpiData: Partial<KPIData>) => {
    if (editingKPI) {
      updateKPI("professional_services", editingKPI.id, {
        ...kpiData,
        color: "bg-orange-500",
  ...
  const handleSaveInitiative = (initiativeData: Partial<InitiativeData>) => {
    if (editingInitiative) {
      updateInitiative("professional_services", editingInitiative.id, {
        ...initiativeData,
      });
      setEditingInitiative(null);
    } else {
      addInitiative("professional_services", { ...initiativeData } as Omit<
        InitiativeData,
        "id"
      >);
    }
  ...
  const handleDeleteKPI = (id: string) => {
    if (confirm("Are you sure you want to delete this KPI?")) {
      deleteKPI("professional_services", id);
    }
  };

  const handleDeleteInitiative = (id: string) => {
    if (confirm("Are you sure you want to delete this initiative?")) {
      deleteInitiative("professional_services", id);
    }
  };
        ...
        <div className="flex items-center mb-8">
          ...
          <div>
            ...
            <p className="text-lg text-gray-600">
              Manage KPIs and initiatives for professional services performance
            </p>
          </div>
        </div>
