...
import { UserCheck, Plus, Edit, Trash2, BarChart3, Target } from 'lucide-react';
import KPIForm from '../../components/admin/KPIForm';
import InitiativeForm from '../../components/admin/InitiativeForm';
import { useData } from '../../context/DataContext';

export default function HumanResourcesAdminPage() {
  ...
  const kpis = businessUnits.humanResources.kpis;
  const initiatives = businessUnits.humanResources.initiatives;

  const handleSaveKPI = (kpiData: any) => {
    if (editingKPI) {
      updateKPI('humanResources', { ...kpiData, id: editingKPI.id, color: 'bg-pink-500', isVisibleOnDashboard: editingKPI.isVisibleOnDashboard || false });
      setEditingKPI(null);
    } else {
      addKPI('humanResources', { ...kpiData, color: 'bg-pink-500', isVisibleOnDashboard: false });
    }
  ...
  const handleSaveInitiative = (initiativeData: any) => {
    if (editingInitiative) {
      updateInitiative('humanResources', { ...initiativeData, id: editingInitiative.id });
      setEditingInitiative(null);
    } else {
      addInitiative('humanResources', initiativeData);
    }
  ...
  const handleDeleteKPI = (id: number) => {
    if (confirm('Are you sure you want to delete this KPI?')) {
      deleteKPI('humanResources', id);
    }
  };

  const handleDeleteInitiative = (id: number) => {
    if (confirm('Are you sure you want to delete this initiative?')) {
      deleteInitiative('humanResources', id);
    }
  };
    ...
    <div className="p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center mb-8">
          <div className="w-12 h-12 bg-gradient-to-br from-pink-500 to-pink-600 rounded-lg flex items-center justify-center mr-4">
            <UserCheck className="w-6 h-6 text-white" />
          </div>
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Human Resources Admin</h1>
            <p className="text-lg text-gray-600">Manage KPIs and initiatives for human resources performance</p>
          </div>
        </div>
        ...
        {activeTab === 'kpis' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h2 className="text-xl font-semibold text-gray-900">Human Resources KPIs</h2>
              <button
        ...
        {activeTab === 'initiatives' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h2 className="text-xl font-semibold text-gray-900">Human Resources Initiatives</h2>
              <button
