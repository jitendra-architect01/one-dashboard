...
import { Code, Plus, Edit, Trash2, BarChart3, Target } from "lucide-react";
...
export default function ProductEngineeringAdminPage() {
  ...
  const kpis = businessUnits.product_engineering?.kpis || [];
  const initiatives = businessUnits.product_engineering?.initiatives || [];

  const handleSaveKPI = (kpiData: Partial<KPIData>) => {
    if (editingKPI) {
      updateKPI("product_engineering", editingKPI.id, {
        ...kpiData,
        color: "bg-indigo-500",
  ...
  const handleSaveInitiative = (initiativeData: Partial<InitiativeData>) => {
    if (editingInitiative) {
      updateInitiative("product_engineering", editingInitiative.id, {
        ...initiativeData,
      });
      setEditingInitiative(null);
    } else {
      addInitiative("product_engineering", { ...initiativeData } as Omit<
        InitiativeData,
        "id"
      >);
    }
  ...
  const handleDeleteKPI = (id: string) => {
    if (confirm("Are you sure you want to delete this KPI?")) {
      deleteKPI("product_engineering", id);
    }
  };

  const handleDeleteInitiative = (id: string) => {
    if (confirm("Are you sure you want to delete this initiative?")) {
      deleteInitiative("product_engineering", id);
    }
  };
        ...
        <div className="flex items-center mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              Product & Engineering Admin
            </h1>
            <p className="text-lg text-gray-600">
              Manage KPIs and initiatives for product and engineering
              performance
            </p>
          </div>
        </div>
