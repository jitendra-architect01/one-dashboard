...
import { FileText, Plus, Edit, Trash2, BarChart3, Target } from "lucide-react";
...
export default function GeneralAdministrativeAdminPage() {
  ...
  const kpis = businessUnits.general_administrative?.kpis || [];
  const initiatives = businessUnits.general_administrative?.initiatives || [];

  const handleSaveKPI = (kpiData: Partial<KPIData>) => {
    if (editingKPI) {
      updateKPI("general_administrative", editingKPI.id, {
        ...kpiData,
        color: "bg-gray-500",
  ...
  const handleSaveInitiative = (initiativeData: Partial<InitiativeData>) => {
    if (editingInitiative) {
      updateInitiative("general_administrative", editingInitiative.id, {
        ...initiativeData,
      });
      setEditingInitiative(null);
    } else {
      addInitiative("general_administrative", { ...initiativeData } as Omit<
        InitiativeData,
        "id"
      >);
    }
  ...
  const handleDeleteKPI = (id: string) => {
    if (confirm("Are you sure you want to delete this KPI?")) {
      deleteKPI("general_administrative", id);
    }
  };

  const handleDeleteInitiative = (id: string) => {
    if (confirm("Are you sure you want to delete this initiative?")) {
      deleteInitiative("general_administrative", id);
    }
  };
        ...
        <div className="flex items-center mb-8">
          ...
          <div>
            ...
            <p className="text-lg text-gray-600">
              Manage KPIs and initiatives for administrative functions and operational support
            </p>
          </div>
        </div>
